import BookingEmbed from '@/components/BookingEmbed';
import Header from '@/components/Header';
import Footer from '@/components/Footer';
import Button from '@/components/Button';
import Link from 'next/link';

export const metadata = {
  title: "Children's Health & Pediatric Care | Atlas Integrative Clinic",
  description: "Gentle, natural healthcare for children. Safe and effective treatments for common childhood conditions, immune support, and developmental wellness.",
};

export default function ChildrensHealthPage() {
  const conditions = [
    "Common colds and flu",
    "Digestive issues",
    "Allergies and asthma",
    "Sleep disturbances",
    "ADHD and behavioral concerns",
    "Growing pains",
    "Immune system support",
    "Developmental wellness"
  ];

  const benefits = [
    {
      title: "Gentle Care",
      description: "Non-invasive, child-friendly treatments that are safe and comfortable",
      icon: (
        <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/>
        </svg>
      )
    },
    {
      title: "Stronger Immunity",
      description: "Build natural defenses and reduce frequency of illness",
      icon: (
        <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z"/>
        </svg>
      )
    },
    {
      title: "Natural Healing",
      description: "Support the body's innate ability to heal without side effects",
      icon: (
        <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/>
        </svg>
      )
    },
    {
      title: "Holistic Wellness",
      description: "Address root causes for lasting health and vitality",
      icon: (
        <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M14.828 14.828a4 4 0 01-5.656 0M9 10h.01M15 10h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/>
        </svg>
      )
    }
  ];

  return (
    <>
      <Header />
      <main className="overflow-hidden">
        {/* Breadcrumbs */}
        <section className="bg-sage-50 py-4 border-b border-sage-100">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <nav className="flex items-center gap-2 text-sm">
              <Link href="/" className="text-gray-600 hover:text-black transition-colors">Home</Link>
              <svg className="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7"/>
              </svg>
              <Link href="/what-we-treat" className="text-gray-600 hover:text-black transition-colors">What We Treat</Link>
              <svg className="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7"/>
              </svg>
              <span className="text-black font-medium">Children's Health & Pediatric Care</span>
            </nav>
          </div>
        </section>

        {/* Hero Section */}
        <section className="relative py-16 lg:py-24 bg-gradient-to-br from-sage-50 via-white to-sage-50">
          <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
            <h1 className="font-serif text-4xl lg:text-5xl font-light text-gray-900 mb-6 leading-tight">
              Children's Health & <span className="italic text-black">Pediatric Care</span>
            </h1>
            <p className="text-xl text-gray-600 leading-relaxed">
              Gentle, effective natural healthcare for children—supporting healthy development, building
              immunity, and addressing common childhood conditions with safe, compassionate care.
            </p>
          </div>
        </section>

        {/* Overview Section */}
        <section className="py-16 lg:py-20 bg-white">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="grid lg:grid-cols-2 gap-12 items-center">
              <div>
                <h2 className="font-serif text-3xl lg:text-4xl font-light text-gray-900 mb-6">
                  Gentle Pediatric <span className="italic text-black">Wellness</span>
                </h2>
                <div className="space-y-4 text-gray-600 leading-relaxed">
                  <p>
                    Children respond beautifully to natural medicine, often experiencing faster healing and
                    fewer side effects than with conventional treatments alone. Our pediatric care provides
                    gentle, child-friendly therapies that support healthy growth, strengthen immunity, and
                    address common childhood health concerns.
                  </p>
                  <p>
                    We use specialized techniques including gentle acupuncture (or non-needle alternatives for
                    younger children), pediatric massage, herbal medicine, and nutritional guidance. Our approach
                    is always tailored to your child's age, temperament, and specific health needs, creating a
                    comfortable, positive healing experience.
                  </p>
                  <p>
                    From recurrent colds and digestive issues to sleep problems and behavioral challenges, we
                    work with families to address root causes and build lasting health—empowering children to
                    thrive naturally.
                  </p>
                </div>
              </div>

              <div className="relative">
                <div className="aspect-[1402/1122] rounded-2xl overflow-hidden bg-gradient-to-br from-sage-100 to-sage-200">
                  <img
                    src="/images/treats/childrens-health.jpg"
                    alt="Practitioner providing gentle back acupressure to a child"
                    className="w-full h-full object-cover"
                  />
                </div>
              </div>
            </div>
          </div>
        </section>

        {/* Conditions Section */}
        <section className="py-16 lg:py-20 bg-gradient-to-br from-gray-50 to-sage-50">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center mb-12">
              <h2 className="font-serif text-3xl lg:text-4xl font-light text-gray-900 mb-4">
                Conditions We <span className="italic text-black">Treat</span>
              </h2>
              <p className="text-lg text-gray-600 max-w-2xl mx-auto">
                Our gentle treatments support a wide range of pediatric health concerns
              </p>
            </div>

            <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {conditions.map((condition) => (
                <div key={condition} className="bg-white rounded-xl p-4 border border-sage-100 hover:border-sage-300 hover:shadow-md transition-all">
                  <div className="flex items-center gap-3">
                    <svg className="w-5 h-5 text-black flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                      <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd"/>
                    </svg>
                    <span className="text-gray-700 font-medium">{condition}</span>
                  </div>
                </div>
              ))}
            </div>
          </div>
        </section>

        {/* Approach Section */}
        <section className="py-16 lg:py-20 bg-white">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center mb-12">
              <h2 className="font-serif text-3xl lg:text-4xl font-light text-gray-900 mb-4">
                Our <span className="italic text-black">Approach</span>
              </h2>
              <p className="text-lg text-gray-600 max-w-3xl mx-auto">
                A gentle, family-centered approach to children's health and development
              </p>
            </div>

            <div className="grid md:grid-cols-3 gap-8">
              <div className="bg-gradient-to-br from-sage-50 to-white p-8 rounded-2xl border border-sage-100">
                <div className="w-12 h-12 bg-sage-600 rounded-xl flex items-center justify-center mb-6">
                  <svg className="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"/>
                  </svg>
                </div>
                <h3 className="font-serif text-xl font-semibold text-gray-900 mb-3">
                  Child-Centered Assessment
                </h3>
                <p className="text-gray-600 leading-relaxed">
                  Comprehensive evaluation that considers your child's development, health history, and family wellness.
                </p>
              </div>

              <div className="bg-gradient-to-br from-sage-50 to-white p-8 rounded-2xl border border-sage-100">
                <div className="w-12 h-12 bg-sage-600 rounded-xl flex items-center justify-center mb-6">
                  <svg className="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19.428 15.428a2 2 0 00-1.022-.547l-2.387-.477a6 6 0 00-3.86.517l-.318.158a6 6 0 01-3.86.517L6.05 15.21a2 2 0 00-1.806.547M8 4h8l-1 1v5.172a2 2 0 00.586 1.414l5 5c1.26 1.26.367 3.414-1.415 3.414H4.828c-1.782 0-2.674-2.154-1.414-3.414l5-5A2 2 0 009 10.172V5L8 4z"/>
                  </svg>
                </div>
                <h3 className="font-serif text-xl font-semibold text-gray-900 mb-3">
                  Gentle Treatments
                </h3>
                <p className="text-gray-600 leading-relaxed">
                  Age-appropriate therapies including pediatric acupuncture, massage, and herbal medicine for safe healing.
                </p>
              </div>

              <div className="bg-gradient-to-br from-sage-50 to-white p-8 rounded-2xl border border-sage-100">
                <div className="w-12 h-12 bg-sage-600 rounded-xl flex items-center justify-center mb-6">
                  <svg className="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"/>
                  </svg>
                </div>
                <h3 className="font-serif text-xl font-semibold text-gray-900 mb-3">
                  Family Partnership
                </h3>
                <p className="text-gray-600 leading-relaxed">
                  Parent education and home care guidance to support your child's ongoing health and development.
                </p>
              </div>
            </div>
          </div>
        </section>

        {/* Benefits Section */}
        <section className="py-16 lg:py-20 bg-gradient-to-br from-gray-50 to-sage-50">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center mb-12">
              <h2 className="font-serif text-3xl lg:text-4xl font-light text-gray-900 mb-4">
                Treatment <span className="italic text-black">Benefits</span>
              </h2>
            </div>

            <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
              {benefits.map((benefit) => (
                <div key={benefit.title} className="bg-white p-6 rounded-xl border border-sage-100 hover:shadow-lg transition-shadow">
                  <div className="w-12 h-12 bg-sage-100 rounded-full flex items-center justify-center mb-4 text-black">
                    {benefit.icon}
                  </div>
                  <h3 className="font-semibold text-gray-900 mb-2">{benefit.title}</h3>
                  <p className="text-sm text-gray-600">{benefit.description}</p>
                </div>
              ))}
            </div>
          </div>
        </section>

        {/* CTA Section */}
        <section className="relative py-20 lg:py-24 bg-gradient-to-br from-sage-600 via-sage-700 to-sage-800 overflow-hidden">
          <div className="absolute inset-0 opacity-10">
            <div className="absolute top-0 left-1/4 w-96 h-96 bg-white rounded-full blur-3xl" />
            <div className="absolute bottom-0 right-1/4 w-96 h-96 bg-sage-400 rounded-full blur-3xl" />
          </div>

          <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10">
            <h2 className="font-serif text-4xl lg:text-5xl font-light text-white mb-6 leading-tight">
              Ready to Support Your Child's <span className="italic">Health?</span>
            </h2>
            <p className="text-xl text-white mb-10 max-w-2xl mx-auto leading-relaxed">
              Schedule a consultation to discover how gentle, natural pediatric care can help your child thrive and grow healthy.
            </p>

            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <BookingEmbed />
              <Button href="/contact" variant="outline" size="lg" className="border-2 border-white/30 hover:bg-white/10 text-white backdrop-blur-sm">
                Contact Us
              </Button>
            </div>
          </div>
        </section>

        {/* Back Link */}
        <section className="py-8 bg-white border-t border-sage-100">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <Link href="/what-we-treat" className="inline-flex items-center gap-2 text-black hover:text-black font-medium group">
              <svg className="w-4 h-4 group-hover:-translate-x-1 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7"/>
              </svg>
              <span>Back to What We Treat</span>
            </Link>
          </div>
        </section>
      </main>
      <Footer />
    </>
  );
}
